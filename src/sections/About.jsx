...
function About() {
  ...
  return (
    <section id='about' className='!h-auto flex-center relative md:p-5 px-5 '>
            ...
            <div id='card' className="md:col-span-6 col-span-12 row-span-3">
              <div className='bg-black-300 rounded-2xl p-7 w-full h-full'>
                <div className='flex flex-col h-full justify-center'>{ }
                  <h1 className='gradient-title md:text-2xl text-xl font-bold animate-text'>Web Design & Dev</h1>
                  <p className='md:text-xl max-w-96 animate-text'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Tempore, accusamus ipsa temporibus soluta obcaecati ab.</p>
                </div>
              </div>
            </div>
            {/* web design cars{} */}
            ...
            <div id='card' className="md:col-span-4 col-span-12 row-span-4">
              <div className='bg-black-300 rounded-2xl p-7 w-full h-full'>
                <div className="flex flex-col justify-between h-full">{["BE YOURSELF", "BE DIFRENT", "BUILD DIFFRENT"].map((text, indez) => (
                  <h1 key={indez} className='gradient-title md:text-3xl animate-text text-2xl font-bold'>{text}</h1>
                ))}</div>
              </div>
            </div>
            {/* brnto social inks */}
              ...
              return (
                <div key={index} className='md:col-span-4 col-span-12 row-span-2'>
                  <div className="bg-black-300 rounded-2xl p-7 w-full h-full group cursor-pointer">
                    <div id='about' className="flex justify-between items-center h-full">
                      <div className='flex items-center md:gap-5'>
                        <Icon size={40} />
                        <h1 className='gradient-title md:text-2xl text-xl animate-text md:m-0 ms-5 font-extrabold'>{item.name}</h1>
                      </div>
                      <div className="lg:block md:hidden group-hover:translate-y-2 transition-transform">
                        <img src={arrow} className='size-7 lg:scale-100 scale-50' alt="arrow down" />
                      </div>
                    </div>
                  </div>
                </div>
